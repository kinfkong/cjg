...
- (void)loadView {
    ...
    if (self.info == nil && self.bookId != nil) {
        //NSLog(@"execute here");
        NSString* urlstr = [NSString stringWithFormat:@"http://ishelf.obanana.com/index.php/mybook/get/%@/%@", self.bookId, @"dali"];
        ...
        NSURLRequest* urlrequest = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://ishelf.obanana.com/index.php/isbn/index/%@/%@", self.isbn, @"dali"]]];
        KKNetworkLoadingView* loadingView = [[KKNetworkLoadingView alloc] initWithRequest:urlrequest delegate:self tag:3456];
        [self.view addSubview:loadingView];
        [loadingView release];
...
-(void) statusUpdate:(NSString *) status {
    ...
    NSURLRequest* request = [utilModel generateRequesWithURL:@"http://ishelf.obanana.com/index.php/mybook/updatestatus" 
